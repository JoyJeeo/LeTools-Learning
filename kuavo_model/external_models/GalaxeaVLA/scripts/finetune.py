...
def finetune(cfg: DictConfig):
    ...
    # can run without access to the original pretrained model or processor path.
    if not _dry_run and accelerator.is_main_process:
        _processor_path = (
            cfg.model.model_arch.get("hf_processor_path", None)
            or cfg.model.model_arch.get("pretrained_model_path", None)
        )
        if _processor_path:
            copy_hf_processor_files(_processor_path, output_dir / "hf_processor")
    accelerator.wait_for_everyone()
