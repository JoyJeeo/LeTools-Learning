...
def _apply_hf_processor_sidecar(cfg: DictConfig, run_dir: Path):
    """Resolve the HF processor from the run sidecar or an accessible config path."""
    local_hf = Path(run_dir) / "hf_processor"
    has_local = (local_hf / "tokenizer.json").is_file()

    configured_paths = [
        OmegaConf.select(cfg, "model.model_arch.hf_processor_path"),
        OmegaConf.select(cfg, "model.model_arch.pretrained_model_path"),
        OmegaConf.select(cfg, "model.processor.tokenizer_params.pretrained_model_name_or_path"),
    ]
    resolved_hf = str(local_hf) if has_local else next(
        (str(Path(path).expanduser()) for path in configured_paths if path and Path(path).expanduser().exists()),
        None,
    )

    if resolved_hf is not None:
        cfg.model.model_arch.hf_processor_path = resolved_hf
        cfg.model.model_arch.pretrained_model_path = None
        if OmegaConf.select(cfg, "model.processor.tokenizer_params") is not None:
            cfg.model.processor.tokenizer_params.pretrained_model_name_or_path = resolved_hf
...
def _apply_action_tokenizer_sidecar(cfg: DictConfig, run_dir: Path) -> bool:
    ...
    patched = False
    # ``model.tokenizer`` normally aliases the root node via ``${tokenizer}``.
    # Updating through that alias materializes only the written subtree and drops
    # sibling fields such as ``_target_``, breaking later interpolations. Patch the
    # root source of truth when present; use the model-local layout only for legacy
    # configs that do not have a root tokenizer node.
    tokenizer_key = (
        "tokenizer.vq_config.ckpt_dir"
        if OmegaConf.select(cfg, "tokenizer") is not None
        else "model.tokenizer.vq_config.ckpt_dir"
    )
    keys = [tokenizer_key]
    if OmegaConf.select(cfg, "tokenizer") is None:
        keys.append("model.model_arch.AT_CONFIG.ckpt_dir")
    for key in keys:
