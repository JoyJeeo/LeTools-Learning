...
class XR0LeRobotDataset(JsonDataset):
    """Config-driven LeRobot dataset for XR0 post-training."""
    ...
    def __init__(self, params):
        ...
        self.dataset = LeRobotDataset(
            repo_id=repo_id,
            root=root,
            delta_timestamps={self.layout.action_key: [step / fps for step in range(self.action_length)]},
    ...
    def __getitem__(self, index):
        ...
        is_pad = self._numpy(item[f"{self.layout.action_key}_is_pad"])
        ...
        return {
            ...
            "action_mask": torch.from_numpy(self.layout.action_mask(self.action_length, ~is_pad.astype(bool))),
            "state": torch.from_numpy(self.layout.pack_state(state)),
        }
