...
class ActionLayout:
    """Pack a flat robot state/action space into XR1 model tensors."""
    ...
    @classmethod
    def _components(
        cls, values: Mapping, model_dim: int, is_action: bool
    ) -> tuple[Component, ...]:
        components = []
    ...
    def pack_action(self, state, future_action) -> np.ndarray:
        ...
        if future.ndim != 2 or future.shape[1] != self.action_source_dim:
            raise ValueError(
                f"{self.action_key} expected shape [T, {self.action_source_dim}], got {future.shape}"
            )
    ...
    def recover_action(self, action, state) -> np.ndarray:
        ...
        if action.ndim != 2 or action.shape[1] != self.action_dim:
            raise ValueError(
                f"Model action expected shape [T, {self.action_dim}], got {action.shape}"
            )
    ...
    def action_mask(self, action_length: int, temporal_mask=None) -> np.ndarray:
        temporal = (
            np.ones(action_length, dtype=np.int32)
            if temporal_mask is None
            else np.asarray(temporal_mask, dtype=np.int32)
        )
        if temporal.shape != (action_length,):
            raise ValueError(
                f"temporal_mask expected shape {(action_length,)}, got {temporal.shape}"
            )
    ...
    def state_mask(self) -> np.ndarray:
        mask = np.zeros((1, self.state_dim), dtype=bool)
        for component in self.state:
